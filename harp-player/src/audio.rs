/// Simple synthesizer for chord playback.
...
impl SynthState {
    ...
    pub fn play_chord(&mut self, midi_notes: &[i32]) {
        self.voices.clear();
        let mut sorted: Vec<i32> = midi_notes.to_vec();
        sorted.sort();
        let strum_gap = 0.03; // 30ms between each string
...
impl AudioPlayer {
    ...
    pub fn play_chord(&self, midi_notes: &[i32]) {
